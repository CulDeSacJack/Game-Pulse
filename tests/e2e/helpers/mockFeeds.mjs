function getRelativeIso(minutesAgo = 0) {
  return new Date(Date.now() - minutesAgo * 60 * 1000).toISOString();
}

const NEWS_FIXTURE_TEMPLATES = new Map([
...
function getNewsItem(feedUrl) {
  const template = NEWS_FIXTURE_TEMPLATES.get(feedUrl);
  if (template) {
    return {
      ...template,
      pubDate: getRelativeIso(template.minutesAgo || 0),
    };
  }
...
function buildSocialPayload(actor) {
  ...
  const fixtureTime = getRelativeIso(3);
